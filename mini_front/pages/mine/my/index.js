...
Page({
  ...
  bmDel(e){ //如果活动报名取消的 进行删除
    let _that = this;
    let order_id = e.currentTarget.dataset.id;
    wx.request({
      url: `${config.del_order}/${order_id}`,
      data:{
        client: 1
      },
      success: function (res) {
  ...
  getMyorder(is_paid) {
    ...
    let parmas = {
      "client":1,
      "mobile": wx.getStorageSync('user_save_infos').mobile,
    ...
    wx.request({
      ...
      success: function(res) {
        if (is_paid==1){
          _that.setData({
            dataList: res.data.data.apply_lists,
            dataList2: []
          });
        }else{
          _that.setData({
            dataList: res.data.data.apply_lists,
            dataList2: res.data.data.student_lists
          });
        }
  ...
  goTrainDetail(e) {
    ...
    if (e.currentTarget.dataset.change){
      if (other){
        wx.navigateTo({
          url: `/pages/mine/my/allInfo/index?order_id=${order_id}&train_id=${train_id}&change=true&other=y`,
        })
      }else{
        wx.navigateTo({
          url: `/pages/mine/my/allInfo/index?order_id=${order_id}&train_id=${train_id}&change=true`,
        })
      }
      ...
      if (other){
        wx.navigateTo({
          url: `/pages/mine/my/allInfo/index?order_id=${order_id}&train_id=${train_id}&other=y`,
        })
      }else{
        wx.navigateTo({
          url: `/pages/mine/my/allInfo/index?order_id=${order_id}&train_id=${train_id}`,
        })
      }
